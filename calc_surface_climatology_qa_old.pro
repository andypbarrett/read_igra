...
PRO CALC_SURFACE_CLIMATOLOGY_QA, PROFILE, TAVERAGE, TSTDDEV, NEXCLUDE, $
  ...
  taverage     = MAKE_ARRAY( 12, 2, /FLOAT, VALUE=-9999.99 )
  tstddev      = MAKE_ARRAY( 12, 2, /FLOAT, VALUE=-9999.99 )
  nexclude     = MAKE_ARRAY( 12, 2, /FLOAT, VALUE=0 )

  thishour = [ 0, 12 ]

  FOR ih = 0, 1 DO BEGIN
     
     hr_profile = GET_HOUR_PROFILE( Profile, thishour[ih], NPROFILE=num_thishour )
     ...
     FOR imon = 0, 11 DO BEGIN
        ...
        mon_profile = GET_MONTH_PROFILE( hr_profile, thismonth, NPROFILE=num_thismonth )
        ...
        IF ( KEYWORD_SET( DEBUG ) EQ 1 ) THEN BEGIN
           PRINT, '%     THISHOUR=' + STRTRIM( thishour[ih], 2 ) + $
               '   THISMONTH=' + STRTRIM( thismonth, 2 ) + $
               '   NUM_THISHOUR='    + STRTRIM( num_thishour, 2 ) + $ 
        ...
        taverage[ imon, ih ] = tavg
        tstddev[ imon, ih ]  = tstd
        nexclude[ imon, ih ] = nexclude_
