...
PRO PLOT_RESULTS, MEAN_MON, STDDEV_MON
  ...
  FOR imon=0, 11 DO BEGIN
     ...
     PLOT, mean_mon[imon,*], level, XRANGE=[0,xmax], XSTYLE=1, $
...
;  idx = WHERE( station.latitude GE 60. and station.latitude LT 70, nstat60 )
  ...
  FOR istat = 0, nstat60 - 1 DO BEGIN
     ...
     station_file = station[ idx[istat] ].station_number + '.month.shum.qc.nc'
     data = NCDF_READV( station_file, VARNAME='shum' )
...
;;  ENDFOR
  ...
  WRITE_RESULTS, mean_mon, stddev_mon, station_mean, 'dai_radiosonde.shum.clm.month.profiles.70Nto90N.' + $
                                       timespan_string + '.nc'
