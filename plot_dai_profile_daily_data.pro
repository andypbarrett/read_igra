...
PRO WRITE_RESULTS, MEAN_MON, STDDEV_MON, STATION_MEAN, STATION_MIN, STATION_MAX, OUTFILE
  ...
  NCDF_ATTPUT, cdfid, avg_varID, 'long_name', 'monthly average QV'
  NCDF_ATTPUT, cdfid, avg_varID, 'units', 'g/kg'
  
  NCDF_ATTPUT, cdfid, sdv_varID, 'long_name', 'monthly standard deviation QV'
  NCDF_ATTPUT, cdfid, sdv_varID, 'units', 'g/kg'
  
  NCDF_ATTPUT, cdfid, sta_varID, 'long_name', 'monthly station average'
  NCDF_ATTPUT, cdfid, sta_varID, 'units', 'g/kg'
  
  NCDF_ATTPUT, cdfid, stn_varID, 'long_name', 'monthly station minimum'
  NCDF_ATTPUT, cdfid, stn_varID, 'units', 'g/kg'
  
  NCDF_ATTPUT, cdfid, stx_varID, 'long_name', 'monthly station maximum'
  NCDF_ATTPUT, cdfid, stx_varID, 'units', 'g/kg'
...
FOR istat = 0, nstat-1 DO BEGIN
   ...
   FOR imon = 0, 11 DO BEGIN
      ...
      xmax = CEIL( MAX( qv ) / 0.5 ) * 0.5 
      
      PLOT, qv, pv, XRANGE=[xmin,xmax], XSTYLE=1, XTITLE='q (g/kg)', $
...
FOR imon = 0, nmonth-1 DO BEGIN
   FOR ilev = 0, nlevel-1 DO BEGIN
      zonalAvg[ imon, ilev ] = MEAN( monAvg_qc[ *, imon, ilev ] )
      zonalStd[ imon, ilev ] = STDDEV( monAvg_qc[ *, imon, ilev ] )
      zonalMin[ imon, ilev ] = MIN( monAvg_qc[ *, imon, ilev ] )
      zonalMax[ imon, ilev ] = MAX( monAvg_qc[ *, imon, ilev ] )
   ENDFOR
ENDFOR
...
;; Write to netCDF
outFile = 'dai_radiosonde.shum.clm.month.profiles.70Nto90N.' + $
          STRTRIM(ybeg,2) + 'to' + STRTRIM(yend,2) + '.20120410.nc'
