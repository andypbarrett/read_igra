...
PRO PLOT_RESULTS, MEAN_MON, STDDEV_MON
  ...
  FOR imon=0, 11 DO BEGIN
     ...
     PLOT, mean_mon[imon,*], level, XRANGE=[0,100.], XSTYLE=1, $
...
PRO WRITE_RESULTS, MEAN_MON, STDDEV_MON, OUTFILE
  ...
  NCDF_ATTPUT, cdfid, avg_varID, 'long_name', 'monthly average RH'
  NCDF_ATTPUT, cdfid, avg_varID, 'units', '%'
  
  NCDF_ATTPUT, cdfid, sdv_varID, 'long_name', 'monthly standard deviation RH'
  NCDF_ATTPUT, cdfid, sdv_varID, 'units', '%'
...
;  idx = WHERE( station.latitude GE 60. and station.latitude LT 70, nstat60 )
  ...
  IF ( nstat60 LE 0 ) THEN BEGIN
     PRINT, '% MAKE_ZONAL_PROFILE_STATISTICS: No stations exist between 60N and 70N'
     RETURN
  ENDIF

  ;; Define array to hold all station data
  all_data = MAKE_ARRAY( nstat60, 2, 6, 156, /FLOAT, VALUE=-9999.99 )

  ;; Read data into all data array
  FOR istat = 0, nstat60 - 1 DO BEGIN

     ;; Read data from station file
     station_file = station[ idx[istat] ].station_number + '.month.rhum.qc.nc'
     ...
     thisyear = WHERE( year GE 1989 AND year LE 2001, num_thisyear )

     ;; Put data in all_data array
     IF ( num_thisyear EQ 156 ) THEN BEGIN 
...
;;  ENDFOR
  ...
  WRITE_RESULTS, mean_mon, stddev_mon, 'dai_radiosonde.month.profile_statistics.70Nto90N.nc'
